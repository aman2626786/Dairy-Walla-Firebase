...
export function SummaryPage() {
  ...
  const handleDownload = () => {
    ...
    doc.text(profile?.businessName || 'DairySetu', pageW / 2, 12, { align: 'center' });
    ...
    doc.text('Generated by DairySetu  •  Smart dairy ordering platform', pageW / 2, y, { align: 'center' });

    // Save
    const filename = `DairySetu-Bill-${label.replace(/\s/g, '-')}-${format(new Date(), 'ddMMMyyyy')}.pdf`;
    doc.save(filename);
    show('PDF downloaded!');
  };
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({ title: `DairySetu Bill — ${label}`, text });
    } else {
